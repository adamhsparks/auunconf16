...
```{r, echo = TRUE, eval = TRUE, error = FALSE, error = FALSE}
devtools::install_git("https://github.com/swish-climate-impact-assessment/awaptools")
library(awaptools)

# weather variables of interest
vars <- c("maxave", "minave", "totals")

# for loop to loop through all variables and download the data
for (measure in vars) {
  get_awap_data(start = "2016-04-19", end = "2016-04-20", measure)
...
AUS <- getData(name = "GADM", download = TRUE, country = "AUS", level = 2)

# Subset Australia data to QLD
QLD <- AUS[AUS@data$NAME_1 == "Queensland", ]
...
Mask and plot the raster stack using Australia. First mask out the ocean area
around Australia, second plot the resulting objects.

```{r, echo = TRUE, eval = TRUE, error = FALSE, warning = FALSE}

AUS_data <- mask(cfiles, AUS)

plot(AUS_data)

```

Now crop and plot, mask and plot the rastersusing QLD to see the difference
between cropping and masking. Cropping only removes the extra extent when using
a spatial object with a raster. Masking sets values outside the area of interest,
e.g. a polygon on Queensland, to NA.
...
# use the mask command to remove data not covering land
QLD_data <- mask(QLD_data, QLD)

# plot the masked objects
plot(QLD_data)
